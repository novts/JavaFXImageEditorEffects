...
public class JavaFXImageEditorEffects extends Application {
    ...
    public void start(Stage stage) {
      ...
      vboxR.setLayoutY(20);     
       ...
       labelDropShadow.selectedProperty().addListener(new ChangeListener<Boolean>() {
        public void changed(ObservableValue<? extends Boolean> ov,
            Boolean old_val, Boolean new_val) {
    if(new_val==true){
        if (imWidth>640){imv.setFitWidth(640);}
        if (imHeight>480){imv.setFitHeight(480);}
       ...
       dshadow.offsetYProperty().bind(sliderOffsetYDShadow.valueProperty()); 
        ...
        if (imWidth>800){imv.setFitWidth(800);}
        if (imHeight>600){imv.setFitHeight(600);}
...
choiceD.getSelectionModel().selectedItemProperty().addListener(
    ...
    dshadow.setColor(Color.web(new_val));
       });
...
choiceIn.getSelectionModel().selectedItemProperty().addListener(
 ...
 labelRef.selectedProperty().addListener(
    ...
    if(new_val==true){
         if(labelPT.selectedProperty().getValue()==true){ 
        if (imWidth>448){imv.setFitWidth(448);}
        if (imHeight>336){imv.setFitHeight(336);}
            colorAdjust.setInput(null); 
            colorAdjust.setInput(reflection); 
          reflection.setInput(pt); 
       }  else{
     ...
     reflection.bottomOpacityProperty().unbind();  
      ...
      initR();
       ...
       vboxR.getChildren().addAll(btnOp,btnR, btnS);       
        ...
        if (imWidth>800){imv.setFitWidth(800);}
        if (imHeight>500){imv.setFitHeight(500);}
      ...
      if (imWidth>640){
       pt.lrxProperty().bind( sliderlrxPT.valueProperty().add(imv.xProperty()).add(imv.fitWidthProperty()));     
       pt.urxProperty().bind( sliderurxPT.valueProperty().add(imv.xProperty()).add(imv.fitWidthProperty()));
      }  else{
          pt.lrxProperty().bind( sliderlrxPT.valueProperty().add(imv.xProperty()).add(im.widthProperty())); 
          pt.urxProperty().bind( sliderurxPT.valueProperty().add(imv.xProperty()).add(im.widthProperty()));
      }
       if (imHeight>480){
           pt.llyProperty().bind( sliderllyPT.valueProperty().add(imv.yProperty()).add(imv.fitHeightProperty())); 
           pt.lryProperty().bind( sliderlryPT.valueProperty().add(imv.yProperty()).add(imv.fitHeightProperty())); 
       }else{
          pt.llyProperty().bind( sliderllyPT.valueProperty().add(imv.yProperty()).add(im.heightProperty()));  
          pt.lryProperty().bind( sliderlryPT.valueProperty().add(imv.yProperty()).add(im.heightProperty())); 
       }
      
     pt.llxProperty().bind( sliderllxPT.valueProperty().add(imv.xProperty()));     
     pt.ulxProperty().bind( sliderulxPT.valueProperty().add(imv.xProperty())); 
     pt.ulyProperty().bind( sliderulyPT.valueProperty().add(imv.yProperty()));     
     pt.uryProperty().bind( slideruryPT.valueProperty().add(imv.yProperty()));  
         }  
...
stack.setPadding(new Insets(50,100,100,50)); 
 }
  if(labelRef.selectedProperty().getValue()==true){
stack.setPadding(new Insets(50,100,300,50)); 
 }
stack.setLayoutX(5);
stack.setLayoutY(5);
            vboxStack.getChildren().clear();
            ...
            vboxStack.setLayoutY(5);
            vboxStack.getChildren().addAll(wv, stack);
            root.getChildren().addAll(vboxStack, vboxIn);
